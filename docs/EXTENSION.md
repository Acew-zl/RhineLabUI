...
本机 `release/extension` 已构建为 0.5.0。在扩展管理页找到 Rhine Lab，点击「重新加载」，再新建标签页。0.5.0 沿用 0.2.0 权限；如果从 0.1.0 更新，可能需要允许书签和站点图标权限。若你之前安装的是 ZIP 解压到其他位置的副本，需要用新版包替换那个目录，或重新加载本项目的 `release/extension`。
...
- OPEN BOOKMARK / Enter 默认在新标签页打开选中的书签，保留导航页与选档状态；「查看档案详情」保留原详情和 360° 模型入口。
- 「备注」是浏览器保存的书签名称，不生成额外描述。空名称保持为空，不再替换为网址或域名；仅有 Logo 的书签也可正常使用。Logo 和备注位于档案顶部朝上的窄面，文字沿顶边单行排列，过长省略；竖直侧面和正面不放书签信息。界面标题悬停和详情仍可查看完整名称。
...
- 桌面预览通过镜头将档案放大约 25%，把选中区域重新放在左下侧；抽取仍只竖直升降。详情回到原有放大比例。顶部渐变集中为搜索栏留白。
